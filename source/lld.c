...
#if defined(LLD_POLL_TOGGLE) && !defined(LLD_POLL_STATUS)
...
* lld_ProgramSuspendOp - Performs Program Suspend Operation
*
* Function perform program suspend operation.
* Function issues program suspend commands and will poll for completion.
*
* RETURNS: DEVSTATUS
*/
...
* Modified by SCH 14/11/2012 - now uses LLD_UINT32 instead of unsigned int
*/
LLD_UINT32 lld_GetDeviceId
(
FLASHDATA * base_addr   /* device base address in system */
)
{
  ...
  id  = (LLD_UINT32)(FLASH_RD(base_addr, 0x0002) & 0x000000FF) << 16;
  id |= (LLD_UINT32)(FLASH_RD(base_addr, 0x001C) & 0x000000FF) <<  8;
  id |= (LLD_UINT32)(FLASH_RD(base_addr, 0x001E) & 0x000000FF)      ;
#else
  id  = (LLD_UINT32)(FLASH_RD(base_addr, 0x0001) & 0x000000FF) << 16;
  id |= (LLD_UINT32)(FLASH_RD(base_addr, 0x000E) & 0x000000FF) <<  8;
  id |= (LLD_UINT32)(FLASH_RD(base_addr, 0x000F) & 0x000000FF)      ;
#endif
  /* Write Software RESET command */
  FLASH_WR(base_addr, 0, NOR_RESET_CMD);
...
#if (defined LLD_CONFIGURATION_X8X16_AS_X8) || (defined LLD_CONFIGURATION_X8_AS_X8)
  id  = (LLD_UINT32)(FLASH_RD(base_addr+offset, 0x0002) & 0x000000FF) << 16;
  id |= (LLD_UINT32)(FLASH_RD(base_addr+offset, 0x001C) & 0x000000FF) <<  8;
  id |= (LLD_UINT32)(FLASH_RD(base_addr+offset, 0x001E) & 0x000000FF)      ;
#else
  id  = (LLD_UINT32)(FLASH_RD(base_addr+offset, 0x0001) & 0x000000FF) << 16;
  id |= (LLD_UINT32)(FLASH_RD(base_addr+offset, 0x000E) & 0x000000FF) <<  8;
  id |= (LLD_UINT32)(FLASH_RD(base_addr+offset, 0x000F) & 0x000000FF)      ;
#endif
...
)
{
  ...
  return( status_reg );          /* return the status reg. */
}


/******************************************************************************
*
* lld_PollWithTimeout - Polls flash device for embedded operation completion
*
*  Function polls the Flash device to determine when an embedded
*  operation has finished - bit 7 is 1.  Uses an adjustable timeout and\or
*  force flag.
*
...
*
*/
FLASHDATA lld_PollWithTimeout
(
FLASHDATA * base_addr,          /* device base address in system */
ADDRESS offset,                 /* address offset from base address */
LLD_UINT32 initial_timeout      /* Initial value for timeout */
)
{
    LLD_UINT32          polling_counter = initial_timeout;
    volatile FLASHDATA  status_reg = 0u;

#ifndef UNIT_TEST_BUILD
    m_b_force_timeout = FALSE;
#endif

    if (initial_timeout != 0u)
    {
...
/******************************************************************************
*
* lld_ForceTimeoutFlagSet - forces a timeout from the above polling function.
*
* This assumes that a higher priority task than is being used to run the flash
* is used to call this function.
*
*/
void lld_ForceTimeoutFlagSet(void)
{
...
/******************************************************************************
*
* lld_ForceTimeoutFlagReset - resets the force timeout flag.
*
* Only used when unit testing, hence the conditional compilation.
*
*/
void lld_ForceTimeoutFlagReset_TDD(void)
{
    m_b_force_timeout = FALSE;
}
...
DEVSTATUS lld_WriteByteBufferProgramOp
(
FLASHDATA * base_addr,          /* device base address is system */
ADDRESS offset,                 /* address offset from base address */
WORDCOUNT word_count,           /* number of WORDS to program */
const BYTE * const p_data_buf   /* buffer containing data to program as BYTES */
)
{
    ADDRESS         last_loaded_addr;
    ADDRESS         current_offset;
    ...
    FLASHDATA       wcount;
    FLASHDATA       status_reg;
    FLASHDATA       temp_write;
    LLD_UINT32      write_offset = 0u;
    ...
    */
    ...
    while(current_offset <= end_offset)
    {
      ...
      /* Write Data, combining 2 x bytes into 1 x word */
      temp_write = (uint16_t)(p_data_buf[write_offset] & 0x00FFu);
      write_offset++;

      temp_write |= (uint16_t)( (p_data_buf[write_offset] << 8u) & 0xFF00u);
      write_offset++;

      FLASH_WR(base_addr, current_offset, temp_write);
      current_offset++;
    }
    ...
    /* Poll with no initial timeout so we use the callback function instead. */
    status_reg = lld_PollWithTimeout(base_addr, last_loaded_addr, 0u);
...
*
* RETURNS: DEVSTATUS
* Modified by SCH 30/06/2016 - add byte version
*/
...
/******************************************************************************
*
* lld_memcpy_bytes   This function attempts to mimic the standard memcpy
*                    function for flash, with an 8 bit data input buffer.
*                    It segments the source data into page size chunks for use
*                    by Write Buffer Programming.
*
*                    Note that the lld_WriteByteBufferProgramOp function polls
*                    the flash using the callback timeout.
*
* RETURNS: DEVSTATUS
*/
DEVSTATUS lld_memcpy_bytes
(
FLASHDATA * base_addr,          /* device base address is system */
ADDRESS offset,                 /* address offset from base address */
WORDCOUNT word_cnt,             /* number of WORDS to program */
...
)
{
    ...
    if (offset & mask)
    {
       ...
       /* program the first few to get write buffer aligned */
       status = lld_WriteByteBufferProgramOp(base_addr,
                                             offset,
                                             intwc,
                                             &p_data_buf[write_offset]);
       ...
       offset       += intwc;           /* adjust pointers and counter */
       word_cnt     -= intwc;
       write_offset += (intwc * 2u);    /* Adjust write offset by number of BYTES */
       if (word_cnt == 0)
       {
         return(status);
       }
    ...
    while(word_cnt >= LLD_BUFFER_SIZE) /* while big chunks to do */
    {
      ...
      offset       += LLD_BUFFER_SIZE;          /* adjust pointers and counter */
      word_cnt     -= LLD_BUFFER_SIZE;
      write_offset += (LLD_BUFFER_SIZE * 2u);   /* Adjust write offset by number of BYTES */
    }
    if (word_cnt == 0)
    {
      return(status);
...
#if  defined(LLD_STATUS_GET) &&  !defined(LLD_POLL_STATUS)
  unsigned long  polling_counter = 0xFFFFFFFF;
#endif
#ifdef LLD_POLL_STATUS
    FLASHDATA    status_reg;
