...
#define LLD_VERSION   "12.3.0"   //  Year.Quarter.Minor

/* LLD System Specific Typedefs */
typedef uint8_t        BYTE;        /* 8 bits wide */
typedef char_t         LLD_CHAR;    /* 8 bit wide */
typedef uint16_t       LLD_UINT16;  /* 16 bits wide */
typedef uint32_t       LLD_UINT32;  /* 32 bits wide */
typedef uint64_t       LLD_UINT64;  /* 64 bits wide */
typedef LLD_UINT32     ADDRESS;     /* Used for system level addressing */
typedef LLD_UINT16     WORDCOUNT;   /* used for multi-byte operations */
...
#include "extflash.h"			/* FLI specific flash read and write functions */
...
/* FLI specific defines for flash chips - this is a bodge to get around the
 * fact that pointers on the 28335 are only 16 or 22 bit (near and far) so
 * cannot be used for addressing the flash memory - please refer to
 * SPRU514E section 6.4 for more information */
#define DEVICE_ZERO_BASE	(FLASHDATA*)0
#define DEVICE_ONE_BASE		(FLASHDATA*)1
        ...
        #define FLASH_RD(b,o)           FlashRead(b,o)
            ...
            #define FLASH_RD(b,o)       FlashRead_Extend(b,o) 
				...
				/* FLI specific write and read functions - this is where we
				 * adjust the base address depending on whether we're writing
				 * to flash zero or flash one.  It's a bit nasty, but has to
				 * be done like this because of the pointer limitations
				 * described above.
				 */
                #define FLASH_WR(b,o,d) ( (b == DEVICE_ZERO_BASE) ? EXTFLASH_ExternalFlashWrite( 0x00000000u + (ADDRESS)o, d) : EXTFLASH_ExternalFlashWrite( 0x04000000u + (ADDRESS)o, d) )
                #define FLASH_RD(b,o)   ( (b == DEVICE_ZERO_BASE) ? EXTFLASH_ExternalFlashRead ( 0x00000000u + (ADDRESS)o)    : EXTFLASH_ExternalFlashRead ( 0x04000000u + (ADDRESS)o)    )
            #else
...
extern DEVSTATUS lld_WriteByteBufferProgramOp
(
FLASHDATA * base_addr,          /* device base address is system */
ADDRESS offset,                 /* address offset from base address */
WORDCOUNT word_count,           /* number of words to program */
...
#ifdef LLD_MEMCPY_OP			/* Modified by SCH 30/06/2016 - add byte version */
...
extern FLASHDATA lld_PollWithTimeout
(
FLASHDATA * base_addr,          /* device base address in system */
ADDRESS offset,                 /* address offset from base address */
...
extern void lld_ForceTimeoutFlagSet(void);

#ifdef UNIT_TEST_BUILD
extern void lld_ForceTimeoutFlagReset_TDD(void);
#endif

#endif

#ifdef LLD_GET_ID_CMD_1
extern LLD_UINT32 lld_GetDeviceId
(
FLASHDATA * base_addr			/* device base address is system */
);
#endif
